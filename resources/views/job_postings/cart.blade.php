...
@section('content')
    ...
    overflow: hidden;
    width: 120px;
    height: 30px; /* đảm bảo chiều cao cố định để align-items hoạt động */
}

.quantity-btn {
    width: 30px;
    height: 100%; /* full chiều cao để căn giữa */
    background: #11ef8b;
    border: none;
    font-size: 18px;
    cursor: pointer;
...
.quantity-input {
    ...
    text-align: center;
    font-size: 14px;
    color: #000; /* đổi màu text thành đen */
}
            ...
            <div data-v-1eb6ef20="" data-v-48257136="">
                @foreach ($typeservices as $typeservice)
                    <div class="pricing-section">
                        ...
                        @forelse ($typeservice->services as $service)
                                            ...
                                            <select class="week-selector">
                                                @if ($service->weeks->count())
                                                    @foreach ($service->weeks as $week)
                                                        <option value="{{ $week->number_of_weeks }}">
                                                            {{ $week->number_of_weeks }} tuần
                                                        </option>
                                                    @endforeach
                                                @else
                        ...
                        @empty
                            <p>Không có dịch vụ nào cho loại này.</p>
                        @endforelse
                        ...
                        buttons.forEach(button => {
                            button.addEventListener('click', function () {
                                ...
                                if (button.textContent === '-') {
                                    value = Math.max(1, value - 1); // không giảm dưới 1
                                } else {
                                    value += 1;
                                }

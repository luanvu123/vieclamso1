                    ...
                    <table class="display" id="user-table">
                        ...
                        <tbody>
                            @foreach ($companies as $company)
                                @php
                                    // Kiểm tra nếu created_at của công ty chưa tới 2 giờ kể từ hiện tại
                                    $isNew = $company->created_at->diffInHours(now()) < 2;
                                @endphp
                                <tr>
                                    ...
                                    <td @if ($isNew) style="font-weight: bold;" @endif>
                                        <img src="{{ $company->logo ? asset('storage/' . $company->logo) : asset('storage/avatar/avatar-default.jpg') }}"
                                            alt="{{ $company->name }}" width="50">
                                    </td>
                                    <td @if ($isNew) style="font-weight: bold;" @endif>
                                        {{ $company->name }}</td>
                                    <td @if ($isNew) style="font-weight: bold;" @endif>
                                        {{ $company->employer->name }}</td>
                                    <td @if ($isNew) style="font-weight: bold;" @endif>
                                        {{ $company->address }}</td>
                                    <td @if ($isNew) style="font-weight: bold;" @endif>
                                        {{ $company->website }}</td>
                                    <td>
                                        <div class="checkbox-wrapper">
                                            <input type="checkbox" id="top_chooseCheckbox_{{ $company->id }}"
                                                class="top_choose_checkbox" value="{{ $company->top }}"
                                                {{ $company->top == 1 ? 'checked' : '' }}>
                                    ...
                                    <td>
                                        <div class="checkbox-wrapper">
                                            <input type="checkbox" id="top_homeCheckbox_{{ $company->id }}"
                                                class="top_home_checkbox" value="{{ $company->top_home }}"
                                                {{ $company->top_home == 1 ? 'checked' : '' }}>
                                    ...
                                    <td>
                                        <div class="checkbox-wrapper">
                                            <input type="checkbox" id="featuredCheckbox_{{ $company->id }}"
                                                class="featured_checkbox" value="{{ $company->featured }}"
                                    ...
                                    <td>
                                        <label class="plane-switch">
                                            <input type="checkbox" id="statusCheckbox_{{ $company->id }}"
                                                class="plane-switch" value="{{ $company->status ? '1' : '0' }}"
                                    ...
                                    <td>
                                        <a href="{{ route('admin.companies.show', $company->id) }}"><i

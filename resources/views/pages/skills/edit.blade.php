...
<form method="POST" action="{{ route('skills.update', $skill->id) }}">
    ...
    <div class="mb-3">
        <label class="form-label" for="name">Tên kỹ năng *</label>
        <input class="form-control" id="name" type="text" name="name" value="{{ $skill->name }}" required>
    </div>

    <div class="mb-3">
        <label class="form-label" for="proficiency">Trình độ</label>
        <input class="form-control" id="proficiency" type="number" name="proficiency" value="{{ $skill->proficiency }}" min="1" max="10">
    </div>
